...
tic
while(true)
    seed = nanmean(array); % Initialize seed Point.
    ...
    while(true)
        ...
        distth = (sqrt(nansum((array-seed).^2)/numel(array)));% Find bandwidth for Cluster Center.
        %         distth = max(dist(:))/5;
        qualified = dist<distth;% Check values are in selected Bandwidth or not.
        newseed = nanmean(array(qualified));% Update mean.
...
%     figure
%     st = ['objects in cluster ',num2str(k)];
%     imshow(segmented_images{k}), title(st);
% end
