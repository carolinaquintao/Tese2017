...
%             if ii >= Ncirc,
                text((ii.*(Ymax-Ymin)./Ncirc.*cos_scale),...
                    ((Ymax-Ymin)./Ncirc.*sin_scale), ...
                    [' ',num2str((Ymin+ii.*(Ymax-Ymin)./Ncirc),2),' ',...
                    '   ',...
                    label_Y],'verticalalignment','bottom','FontSize',7);
%             else
%                 text(ii.*(Ymax-Ymin)./Ncirc.*cos_scale,...
...
%             end
    ...
    function createRadius(Ymax,Ymin)
        ...
        thetaLabel = [[0,30,60,90,120,150,180,210,240,270,300,330,360],[0:30:360]];
        theta = 0:30:360;
%         cs = [-1,0,0,0,0,0;1,0.866025403784439,0.500000000000000,0,-0.500000000000000,-0.866025403784439];%
        cs = [-cosd(theta); cosd(theta)];
%         sn = [0,0,0,0,0,0;0,0.500000000000000,0.866025403784439,1,0.866025403784439,0.500000000000000];%
...
%             'linestyle',':')
        ...
        rt = 1.1*(Ymax-Ymin);
        for iAngle = 1:numel(theta),
            if theta(iAngle) == 0 || theta(iAngle) == 360,
                text(rt*cosd(theta(iAngle)),rt*sind(theta(iAngle)),'0',...
                    'horizontalalignment','center');
            elseif theta(iAngle) == 90,
                text(rt*cosd(theta(iAngle)),rt*sind(theta(iAngle)),'90',...
                    'horizontalalignment','center');
            elseif theta(iAngle) == 180,
                text(rt*cosd(theta(iAngle)),rt*sind(theta(iAngle)),'180',...
                    'horizontalalignment','center');
            elseif theta(iAngle) == 270,
                text(rt*cosd(theta(iAngle)),rt*sind(theta(iAngle)),'270',...
                    'horizontalalignment','center');
            else
